...
#region Überprüfen ob der FormatString gültig ist
        private bool FormatStringValid()
        {
            ...
            formates.Add("n");
            formates.Add("pos");
            formates.AddRange(Enumerable.Range(1, 9).ToArray().Select(x => x.ToString()).ToList());
            ...
            foreach (string item in text)
            {
                ...
                if (!(!txtFormat.Text.Substring(0, 1).Equals("%") && firstIndex))
                {
                    for (int count = 0; count < formates.Count && !StringValid; count++)
                    {
                        if (item.Length >= formates[count].Length && item.Substring(0, formates[count].Length).Equals(formates[count]))
                        {
                            StringValid = true;
                            if (!formates[count].Equals("n"))
                                IfOnlyNumber = false;
                        }
                    }
